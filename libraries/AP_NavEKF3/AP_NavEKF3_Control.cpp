...
void NavEKF3_core::setWindMagStateLearningMode()
{
    ...
    bool magCalRequested =
        ...
        (effectiveMagCal == MagCal::ALWAYS_MODIFIED); // all the time with protection against ground based magnetic interference

    // Deny mag calibration request if we aren't using the compass, it has been inhibited by the user,
    // we do not have an absolute position reference or are on the ground (unless explicitly requested by the user)
    bool magCalDenied = !use_compass() || (effectiveMagCal == MagCal::NEVER) || (onGround && (effectiveMagCal != MagCal::ALWAYS) && (effectiveMagCal != MagCal::ALWAYS_MODIFIED));
