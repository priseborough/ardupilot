...
void NavEKF3_core::FuseDragForces()
{
    ...
    for (uint8_t axis_index = 0; axis_index < 2; axis_index++) {
        ...
        dragTimeout = (imuSampleTime_ms - lastDragPassTime_ms) > frontend->tasRetryTime_ms;

        // if the innovation consistency check fails then don't fuse the sample
        // unless necessary to avoid loss of navigation
        if (dragTestRatio[axis_index] > 1.0f && !(dragTimeout && posTimeout)) {
            return;
        }
