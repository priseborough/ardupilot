...
void NavEKF3_core::FuseAirspeed()
{
    ...
    float vwn;
    float vwe;
    const float R_TAS = sq(constrain_float(frontend->_easNoise, 0.5f, 5.0f) * constrain_float(dal.get_EAS2TAS(), 0.9f, 10.0f));
    float SH_TAS[3];
    float SK_TAS[2];
    Vector24 H_TAS = {};
    float VtasPred;
...
void NavEKF3_core::SelectBetaDragFusion()
{
    ...
    //
    if (storedDrag.recall(dragSampleDelayed,imuDataDelayed.time_ms) && f_timeTrigger && !inhibitWindStates) {
        FuseDragForces();
        prevBetaDragStep_ms = imuSampleTime_ms;
    }
}
...
void NavEKF3_core::FuseDragForces()
{
    ...
    Vector3f bcoef = frontend->_ballisticCoef.get();
	if (bcoef.x < 1.0f && bcoef.y < 1.0f) {
        // nothing to do
		return;
	}
	...
	const float rho = fmaxf(1.225f * sq(dal.get_EAS2TAS()), 0.1f); // air density
	...
	for (uint8_t axis_index = 0; axis_index < 2; axis_index++) {
		...
		if (axis_index == 0) {
            ...
            const float airSpd = sqrtf((2.0f * fabsf(mea_acc)) / (bcoef.x * rho));
            ...
            const float Kacc = fmaxf(1e-1f, rho * bcoef.x * airSpd);
			...
			dragInnovVar.x = (-HK13*HK21*HK24 + HK14*HK21*HK29 + HK15*HK18*HK21 - HK16*HK21*HK30 + HK17*HK21*HK31 - HK20*HK22 + HK22*HK23 + HK26*HK27 - HK27*HK28 + R_ACC);
			if (dragInnovVar.x < R_ACC) {
				return;
			}
			const float HK32 = Kacc / dragInnovVar.x;
			...
			dragInnovVar.y = (HK12*HK19*HK20 + HK14*HK19*HK29 + HK15*HK18*HK19 + HK16*HK19*HK30 - HK17*HK19*HK31 + HK22*HK23 - HK23*HK24 + HK26*HK27 - HK27*HK28 + R_ACC);
			if (dragInnovVar.y < R_ACC) {
				// calculation is badly conditioned
				return;
			}
			const float HK32 = Kacc / dragInnovVar.y;
		...
		const float dragDirection = (rel_wind_body[axis_index] >= 0.0f) ? 1.0f : -1.0f;
		const float predAccel = - (0.5f / bcoef[axis_index]) * rho * sq(rel_wind_body[axis_index]) * dragDirection;
		dragInnov[axis_index] = predAccel - mea_acc;
		dragTestRatio[axis_index] = sq(dragInnov[axis_index]) / (25.0f * dragInnovVar[axis_index]);
        ...
        // correct the state vector
        for (uint8_t j= 0; j<=stateIndexLim; j++) {
            statesArray[j] = statesArray[j] - Kfusion[j] * dragInnov[axis_index];
        }
        stateStruct.quat.normalize();
