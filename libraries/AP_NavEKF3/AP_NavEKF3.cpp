...
*/
void NavEKF3::writeExtNavData(const Vector3f &pos, const Quaternion &quat, const float posCovNE[3], float posErrD, float angErr, uint32_t timeStamp_ms, uint32_t resetTime_ms)
{
    AP::dal().writeExtNavData(pos, quat, posCovNE, posErrD, angErr, timeStamp_ms, resetTime_ms);

    if (core) {
        for (uint8_t i=0; i<num_cores; i++) {
            core[i].writeExtNavData(pos, quat, posCovNE, posErrD, angErr, timeStamp_ms, resetTime_ms);
        }
    }
}
