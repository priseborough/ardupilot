...
bool NavEKF3_core::InitialiseFilterBootstrap(void)
{
    ...
    if (!(frontend->_options & (int32_t)NavEKF3::Options::AlignWithoutGPS) && dal.gps().status(preferred_gps) < AP_DAL_GPS::GPS_OK_FIX_3D) {
...
void NavEKF3_core::ConstrainStates()
{
    ...
    if (frontend->_highAccuracyMask & (1U<<core_index)) {
        stateStruct.gyro_bias.zero();
        stateStruct.accel_bias.zero();
    } else {
