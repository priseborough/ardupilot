...
public:
    ...
    float calc_thrust(float command, float air_density, float velocity_in) const;

    // calculate power of motor
    float calc_power(float thrust, float air_density, float velocity_in) const;
...
private:
    ...
    float diagonal_size;
    float current;
    float thrust_max; // single motor thrust at maximum motor voltage at sea level density and zero wind hover conditions
    float voltage_max;
    Vector3f moment_of_inertia;
    float efficiency; // ratio of power predicted by momnetum disc theory to measured motor input power
    float prop_area;
