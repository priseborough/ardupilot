        ...
        if (sensors[i].healthy) {
            ...
            if (AP::ahrs().airspeed_vector_true(airspeed_vec_bf)) {
                ...
                if (!is_zero(sensors[i].pcoef_xp) && is_positive(airspeed_vec_bf.x)) {
                    pressure_error += sensors[i].pcoef_xp * density * sq(airspeed_vec_bf.x);
                } else if (!is_zero(sensors[i].pcoef_xn) && is_negative(airspeed_vec_bf.x)) {
                    pressure_error += sensors[i].pcoef_xn * density * sq(airspeed_vec_bf.x);
                }
                if (!is_zero(sensors[i].pcoef_yp) && is_positive(airspeed_vec_bf.y)) {
                    pressure_error += sensors[i].pcoef_yp * density * sq(airspeed_vec_bf.y);
                } else if (!is_zero(sensors[i].pcoef_yn) && is_negative(airspeed_vec_bf.y)) {
                    pressure_error += sensors[i].pcoef_yn * density * sq(airspeed_vec_bf.y);
                }
