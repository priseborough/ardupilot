...
void AP_VisualOdom_MAV::handle_pose_estimate(uint64_t remote_time_us, uint32_t time_ms, float x, float y, float z, const Quaternion &attitude, const float posCov[6], float angErr, uint8_t reset_counter, int8_t quality)
{
    ...
    if (consume) {
        AP::ahrs().writeExtNavData(pos, attitude, posCov, angErr, time_ms, _frontend.get_delay_ms(), get_reset_timestamp_ms(reset_counter));
    }
