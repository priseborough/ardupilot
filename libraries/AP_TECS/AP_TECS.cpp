...
void AP_TECS::_update_height_demand(void)
{
    ...
    // Apply first order lag to height demand equivalent to a time constant of 1.9 sec
    // alpha = 0.05 = dt / (dt + tconst) = 0.1 / (0.1 + 1.9)
    ...
    // During landing approach the lag produced by the LPF applied to the height demand causes
    // us to be consistently above the desired glide slope. Compensate for this effect using
    // the estimated height offset produced by the lag and the demanded height rate of change.
    // Apply the compensation gradually at the start of the landing approach.
    // TODO replace the height LPF and this correction with a better zero-lag filter in the future.
    float new_hgt_dem = _hgt_dem_adj;
    if (_flags.is_doing_auto_land) {
        if (hgt_dem_lag_filter_slew < 1) {
            hgt_dem_lag_filter_slew += (0.1f / timeConstant());
        } else {
            hgt_dem_lag_filter_slew = 1;
        }
        const float height_rate = (_hgt_dem_adj - _hgt_dem_adj_last) / 0.1f;
        new_hgt_dem += hgt_dem_lag_filter_slew * height_rate * 1.9f;
    } else {
