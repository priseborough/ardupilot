...
const AP_Param::GroupInfo AP_TECS::var_info[] = {
    ...
    // @Range: 1.0 10.0
    // @User: Advanced
    AP_GROUPINFO("ACCEL_GF", 34, AP_TECS, _accel_gf, 7.0),
        ...
        if (_flags.is_doing_auto_land) {
            ...
            if (_using_airspeed_for_throttle) {
                ...
                max_climb_condition   |= (_vert_accel_dem >= _vertAccLim) &&
                                            !((_flight_stage == AP_FixedWing::FlightStage::TAKEOFF) || (_flight_stage == AP_FixedWing::FlightStage::ABORT_LANDING));
                max_descent_condition |= (_vert_accel_dem <= - _vertAccLim) && !_landing.is_flaring();
            }
...
void AP_TECS::_update_throttle_without_airspeed(int16_t throttle_nudge)
{
    ...
    // so that the throttle mapping adjusts for the effect of flying in rising or sinking air.
