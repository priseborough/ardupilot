...
const AP_Param::GroupInfo AP_TECS::var_info[] = {
    ...
    // @Range: 0.5 1.1
    // @User: Advanced
    AP_GROUPINFO("HGT_ACC_DAMP", 35, AP_TECS, _hgt_accln_damping_ratio, 0.85f),
...
void AP_TECS::constrain_throttle() {
    ...
    if ((_options & OPTION_HGT_ACCLN_CTRL) || (_options & OPTION_INHIBIT_CLIPSTATUS)) {
        _thr_clip_status = clipStatus::NONE;
    }
}
...
void AP_TECS::_update_throttle_without_airspeed(int16_t throttle_nudge, float pitch_trim_deg)
{
    ...
    float pitch_blended;
    if (_options & OPTION_HGT_ACCLN_CTRL) {
                                    ...
                                    float pitch_trim_deg)
{
    ...
    if (_options & OPTION_HGT_ACCLN_CTRL) {
        ...
        if (_ahrs.using_airspeed_sensor() || _use_synthetic_airspeed) {
            ...
            // check for saturation and prevcent integrator windup
...
void AP_TECS::_calc_vert_accel_demand(void)
{
    ...
    AP::logger().WriteStreaming("TEC4","TimeUS,HAD,UEL,DEL,HD,H,HRD,CR,VAL",
                                "Qffffffff",
                                AP_HAL::micros64(),
