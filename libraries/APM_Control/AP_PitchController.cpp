...
//
float AP_PitchController::get_servo_out_accln(float up_accln, float scaler, float pitch_angle_min, float pitch_angle_max)
{
    ...
    const float right_accel = (GRAVITY_MSS - vdot_dem_clipped.z) * tanf(constrain_float(ahrs.get_roll(), -roll_lim_rad, roll_lim_rad));
    ...
    if (!ahrs.airspeed_estimate_true(TAS)) {
        // shouldn't get here in normal flight but if we do pick an airspeed midway bwrween min and max
        TAS = 0.5f * (float)(aparm.airspeed_max + aparm.airspeed_min) * ahrs.get_EAS2TAS();
    } else {
        TAS = constrain_float(TAS, float(aparm.airspeed_min)*ahrs.get_EAS2TAS(), float(aparm.airspeed_max)*ahrs.get_EAS2TAS());
    }
    ...
    float pitch_rate_max_dps = (pitch_angle_max + aparm.pitch_trim - pitch_deg) / gains.tau;
    float pitch_rate_min_dps = (pitch_angle_min + aparm.pitch_trim - pitch_deg) / gains.tau;
