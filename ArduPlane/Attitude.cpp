...
Vector3f Plane::calc_rpy_speed_scaler(void)
{
    ...
    if (ahrs.airspeed_estimate(aspeed)) {
        ...
        Vector3f aspeed_rpy;
        if (is_positive(g2.prop_disc_loading)) {
            ...
            // which is when propwash effects dominate.
            const float disc_loading = MAX(g2.prop_disc_loading * ahrs.get_accel().x, 0.0f);
            // Solve the quadratic for propeller inflow factor a^2 + a = T / (2 * rho * S * V^2)
            // a = inflow factor
            ...
            aspeed_rpy.x = aspeed * (1.0f + 2.0f * inflow_factor * g2.propwash_roll_gain);
            aspeed_rpy.y = aspeed * (1.0f + 2.0f * inflow_factor * g2.propwash_pitch_gain);
            aspeed_rpy.z = aspeed * (1.0f + 2.0f * inflow_factor * g2.propwash_yaw_gain);
        } else {
